...
describes what users get. Pick a format with `-o`, or let cran choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cran <command> -o table   # aligned columns for reading
cran <command> -o jsonl   # one JSON object per line, for piping
cran <command> -o json    # a single JSON array
cran <command> -o csv     # spreadsheet friendly
cran <command> -o tsv     # tab-separated
cran <command> -o url     # just the URL column
cran <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cran <command> --fields id,title,url
```
...
```bash
cran <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cran <command>            # a table, because this is a terminal
cran <command> | wc -l    # JSONL, because this is a pipe
```
