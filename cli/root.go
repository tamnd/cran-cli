// Package cli builds the cran command tree on top of the cran library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: cran.DefaultConfig()}

	root := &cobra.Command{
		Use:   "cran",
		Short: "Browse CRAN R package repository",
		Long: `cran reads the CRAN R package repository through the crandb API.
No key required. Returns records as table, JSON, JSONL, CSV, TSV, or raw.

cran is an independent tool and is not affiliated with CRAN or R Foundation.`,
		SilenceUsage:  true,
		SilenceErrors: true,
